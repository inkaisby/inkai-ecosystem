  ...
  const fetchClaims = async () => {
    ...
    try {
      const response = await fetch('http://localhost:5001/v1/verifications/pending', {
        headers: { 'Authorization': `Bearer ${(localStorage.getItem('inkai_token') || localStorage.getItem('token'))}` }
      });
    ...
    try {
      await fetch(`http://localhost:5001/v1/verifications/${selectedClaim.id}/process`, {
        ...
        headers: { 
          ...
          'Authorization': `Bearer ${(localStorage.getItem('inkai_token') || localStorage.getItem('token'))}`
        },
